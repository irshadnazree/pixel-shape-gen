...
export const useShapeManagement = () => {
  ...
  // Form state
  const [currentShapeType, setCurrentShapeType] = useState<ShapeType>('ellipse');
  ...
  const addShape = useCallback((position: { x: number; y: number }) => {
  ...
  const handleFormSubmit = useCallback((getNewShapePosition: () => { x: number; y: number }) => {
    if (isEditing) {
      updateSelectedShape();
    } else {
  ...
  const moveShape = useCallback((id: number, position: { x: number; y: number }) => {
    setShapes(prev =>
      prev.map(s => (s.id === id ? { ...s, position } : s))
    );
  }, []);

  const handleMoveShapeLayer = useCallback((shapeId: number, direction: string) => {
    setShapes(prevShapes => {
